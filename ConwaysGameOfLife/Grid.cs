...
namespace ConwaysGameOfLife
{
    class Grid : INotifyPropertyChanged
    {
        ...
        private bool showGrid, withCellMargin;
        ...
        public bool WithCellMargin
        {
            get { return withCellMargin; }
            set
            {
                if (withCellMargin = value) return;

                withCellMargin = value;
                NotifyPropertyChanged("WithCellMargin");
            }
        }
        ...
        {
            showGrid = true;
            withCellMargin = false;
            zoomFactor = 1;
            gridCur = new bool[columns, rows];

            setNextCellGeneration = new Action<int>(SetNextCellGeneration);
        }
        ...
        public void SetNextGeneration()
        {
            ...
            gridTmp = new bool[columns, nextGenRowsTmp];

            Parallel.For(0, columns * nextGenRowsTmp, setNextCellGeneration);

            gridCur = gridTmp;
        }

        private void SetNextCellGeneration(int i)
        {
            SetNextCellGeneration(i / nextGenRowsTmp, i % nextGenRowsTmp);
        }

        private void SetNextCellGeneration(int i, int j)
        {
            ...
            int i, int j, Color color, float strokeThickness)
        {
            ...
            if (IsRectInView(rect)) ds.FillRectangle(rect, color);
        }
